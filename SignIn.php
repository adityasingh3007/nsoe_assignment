...
if(!empty($username) && !empty($password) ) {
			$sql="SELECT ID FROM user_calendar_data WHERE Username='$username' AND Password='$password'";
			echo $sql;
			if($result = $connection->query($sql)) {
				if($result->num_rows==0) {
					...
					if($keep_me_logged_in!=false) {
						setcookie('CALENDAR_USER_ID',$user_ID,time()+(86400*30),"/");
					}
					header("Location: index.php");
				}
			}
					...
					<div class="form_">
						...
						<div class="input_container">
						  <form id="signin_form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
							...
							New User ? <b><a href="/">Sign Up</a></b> here
							</div>
							</center>
						</div>
						</form>
